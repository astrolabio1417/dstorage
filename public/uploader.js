window.addEventListener('load', () => {
  ...
  function getParentNodeId() {
    ...
    const parent = u.searchParams.get('node')
    return parent
  }
  ...
  async function createFile(e) {
    ...
    try {
      ...
      const body = { name: formData.get('name'), parent: getParentNodeId() ?? undefined }
      ...
      Toastify({
        ...
        duration: 3000,
        destination: getNodePath(data.id),
        newWindow: true,
        close: true,
        gravity: 'top',
        position: 'right',
  ...
  function uploader(e) {
    ...
    const parentNodeId = getParentNodeId()

    parentNodeId && formData.append('parent', parentNodeId)
